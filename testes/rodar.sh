...
# jsqr decodifica o QR que o qr.ts codifica: o teste de ida e volta precisa de um
# leitor INDEPENDENTE, senao ele so confirma o proprio erro.
npm install --no-save esbuild@0.24.0 jsqr@1.4.0 >/dev/null 2>&1 || true
...
for (const fn of ["cobranca-atende","cobranca-seguir","cobranca-emitidos","cobranca-painel","cobranca-refresh","cobranca-vigia"]) {
...
$ESB painel_puro.ts   --format=esm --outfile=painel_puro.mjs   --log-level=error
$ESB refresh_puro.ts   --format=esm --outfile=refresh_puro.mjs --log-level=error
$ESB vigia_puro.ts     --format=esm --outfile=vigia_puro.mjs     --log-level=error

falhou=0
for t in boleto_pdf qr montar boleto_mensagem assinatura marca conversa emitidos saude fase vigia; do
  printf '%-18s ' "$t"
