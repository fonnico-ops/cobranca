...
for (const fn of ["cobranca-atende","cobranca-seguir","cobranca-emitidos","cobranca-painel","cobranca-titulos-refresh"]) {
...
$ESB titulos-refresh_puro.ts --format=esm --outfile=refresh_puro.mjs --log-level=error

falhou=0
for t in boleto_pdf qr montar boleto_mensagem assinatura marca conversa emitidos saude fase; do
  printf '%-18s ' "$t"
